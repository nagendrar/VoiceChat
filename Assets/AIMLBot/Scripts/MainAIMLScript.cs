...
public class MainAIMLScript : MonoBehaviour
{
    ...
    public TTSSpeakerInputTesting TTSSpeakerInput;
    ...
    /// <summary>
    /// send the question to the robot
    /// </summary>
    public void SendQuestionToRobot(string STTtext)
    {
        InputBox.text = STTtext;
        StartCoroutine("SendQuesToBot");
    }
    ...
    IEnumerator SendQuesToBot()
    {
        if (string.IsNullOrEmpty(InputBox.text) == false)
        {
            ...
            if (OutputBox.text.Equals(answer))
                TTSSpeakerInput.SpeakClick(answer);
        }
    }
